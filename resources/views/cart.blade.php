                    ...
                    <table class="table">
                        ...
                        <tbody>
                            ...
                            <tr>
                                ...
                                <td data-cart_id ="{{$cart->id}}" data-prod_price="{{$cart->product->price}}">
                                    <div class="input-group quantity mt-4" style="width: 100px;">
                                        ...
                                        <input type="text" class="form-control form-control-sm text-center border-0" name = "quantity" value="{{$cart->quantity}}">
                                        <div class="input-group-btn">
                                ...
                                <td>
                                    <p class="mb-0 mt-4 total_price">{{($cart->product->price)*($cart->quantity)}}.00 $</p>
                                </td>
                                <td>
                    ...
                    <div class="col-sm-8 col-md-7 col-lg-6 col-xl-4">
                        <div class="bg-light rounded">
                            ...
                            <div class="py-4 mb-4 border-top border-bottom d-flex justify-content-between">
                                ...
                                <p class="mb-0 pe-4 total">$0.00</p>
                            </div>
...
$(document).on('click', '.cart-plus-minus',  function(){
        ...
        var prod_price  = $(this).parents('td').data('prod_price');
        if(quantity == 0){
        ...
        $.ajax({
            ...
            type: 'GET',
            data: {cart_id, quantity, prod_price},
            success:function(response){
                if(response.success){
                    // toastr.success(response.message);
                    $(document).find('.cart_counter').text(response.cart_counter);        
        ...
        $(this).closest('tr').find('.total_price').text(quantity*prod_price+'.00 $');
    });    
        ...
        function getTotalPayout(){
            $.ajax({
                ...
                success:function(response){
                    ...
                    if(response.sub_total_payout >0){
                        var total_payout = response.sub_total_payout + 5.00;
                        $(document).find('.sub_total').text('$ '+response.sub_total_payout+'.00');
                        $(document).find('.total').text('$ '+total_payout+'.00');
